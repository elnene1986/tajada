...
**Last session checkpoint:** 2026-07-02 (sales-feature build via Cowork-plans / Claude-Code-builds workflow). SHIPPED TO MAIN (pushed): brief 06 deduction counter on Home (`src/utils/deductions.js`, "deducciones potenciales", dark-gold headline after a contrast fix, cross-session dedup), brief 09 CPA share screen + PDF footer attribution (`src/components/ShareSheet.js`, `src/config/links.js`, footer → GH Pages URL), duplicate-import guard (`src/utils/importGuard.js` — session-level + in-batch, both import paths), briefs 06–12 + `briefs/00-roadmap.md` (now the master plan: build order, decisions B1–B5, Plaid parked). Suite was 65/65 green at last commit. **IN FLIGHT, UNCOMMITTED on branch `fix/share-sheet-ios`:** iOS share-sheet fix (Modal onDismiss deferral + guarded safety-net timers, same fix applied to the paywall→export path) + export consolidation on Summary (single primary "Enviar a tu preparador", PDF/CSV demoted to secondary "Solo guardar" links, withUnlock gating unchanged). NEXT: device-test that branch (share sheet appears after modal closes; Cancelar inert; secondary links work) → commit/merge/push → then roadmap phase 4 (minimal Settings screen) — or better, the contador demo first. NOTE: macOS TCC cut off terminal repo access mid-session; fixed via Full Disk Access for Terminal + claude binaries — if "Operation not permitted" reappears, re-check that grant and fully relaunch Terminal. Prior checkpoint (2026-06-29) below. Built 7 features in priority order, smoke-testing after each. ALL DONE: #1 tax-owed "set aside" estimator, #2 quarterly estimated-tax reminders, #3 home-office simplified deduction, #4 inline deductible tips, #5 1099-K reconciliation, #6 receipt photos, #7 automated parser/tax tests. #1–#3, #5, #6 smoke-tested on device. Details in "What the 2026-06-29 session built" below.
...
**⚠️ DEMO BUILD STATE (2026-06-30) — must revert before launch.** To get a shareable Android APK for the contador demo, the project was put in DEMO mode because `react-native-iap@12.16.4` does not compile against React Native 0.81 (Kotlin errors in `RNIapModule.kt`: `Unresolved reference 'currentActivity'` + Activity type mismatch). Changes: (1) **removed `react-native-iap`** from `package.json`; (2) `src/utils/iap.js` now requires it INDIRECTLY (`var moduleName='react-native-iap'; require(moduleName)`) so Metro bundles fine without it and falls back to the stub provider; (3) new **`src/config/demo.js` `DEMO_BUILD = true`** — `SummaryScreen.withUnlock` short-circuits the paywall when true, so PDF/CSV export works without a purchase (good for the demo). **Before production:** set `DEMO_BUILD = false`, run `npx expo install react-native-iap` (or migrate to its Nitro API — note it may STILL not support the RN version, so budget time to upgrade/patch IAP), and re-test the real paywall. Also dropped unused `@react-native-async-storage/async-storage` (was `^3.0.2`, broke the build looking for `org.asyncstorage.shared_storage`) and added `plugins/withIapStoreFlavor.js` (selects the Play flavor — harmless to keep; only matters when react-native-iap is back).
